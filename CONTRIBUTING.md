...
Thanks for your interest in improving **blind-challenge-template**. This document is about contributing to the **template itself** — the shared scoring backend, infrastructure, and Hugging Face app. It is *not* about the individual challenge repositories people generate from it; once you copy this repo for a challenge, that repo is yours to run as you see fit.
...
- **Run it.** Tests, lint, type checks, and — for infra or app changes — an actual local run. "It looked right" is not verification.
...
- **Branch** off `main` with a descriptive name (`fix/leaderboard-tie-break`, `feat/extra-metric`).
- **Match the surrounding code** — naming, docstring style, comment density. Look at nearby files before adding new patterns.
- **Run the checks locally** before pushing:

  ```bash
  ruff check .
  black --check .          # or: ruff format --check .
  mypy backend
  pytest tests/backend/ --cov=backend
  ```

- **Scoring / config changes:** `backend/config.py` and `hf_space/config.py` are the source of truth for endpoints and dataset sizes, and the test fixtures follow them. If you change endpoint names or sizes, regenerate the fixtures under `tests/backend/test_data/` and update `docs/` accordingly. Keep the two `config.py` files in sync.
- **Infrastructure changes** (`opentofu/**`): run `tofu fmt` and `tofu validate`. Nothing organisation-specific belongs in version control — see [`docs/SETUP.md`](docs/SETUP.md) for how per-deployment values are supplied.
- **Docs:** update `README.md`, `docs/`, and the relevant `*/README.md` when behaviour changes.
